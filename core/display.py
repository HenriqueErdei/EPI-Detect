...
_OK = cfg.COLOR_OK
_ALERT = cfg.COLOR_WARN
_INFO = cfg.COLOR_INFO
_DARK = cfg.COLOR_BG
_GRAY = (168, 155, 138)
_WHITE = (248, 238, 230)
...
class Renderer:
    ...
    def _draw_detections(self, img, detections):
        h, w = img.shape[:2]
        for d in detections:
            color = _OK if d.has_vest else _ALERT
    ...
    def _draw_header(self, img):
        ...
        badge = self.mode
        badge_color = _INFO if self.mode == "LIVE" else (160, 60, 160)
        bw, bx = 58, 180
    ...
    def _draw_status_bar(self, img, detections):
        ...
        col_w = w // 3
        items = [
            (f"COM EPI:  {with_vest}", _OK, 20),
            (f"SEM EPI:  {without_vest}", _ALERT, col_w + 20),
