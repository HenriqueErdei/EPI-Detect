...
Fluxo: `yolov8n-pose.pt` → recorte de torso em `core/detector.py` → limiares `VEST_THRESH` / `VEST_SIDE_THRESH` → voto temporal por IoU.
...
2. `python server.py --source webcam` (ou um mp4 / RTSP de teste)
...
6. RTSP: senha não aparece no HTML nem no stdout
